...
class MeControllerSessionsTest {
	...
	/**
	 * HIN-74 asked whether the parameterless route should answer 400 once the
	 * compatibility array is gone. It should not, and it does not have to: both
	 * parameters already carry defaults, so a bare call is the first page — the
	 * same envelope, just without having to say so.
	 */
	@Test
	void aBareCallIsTheFirstPage() {
		stub(new PageImpl<>(List.of(session(CURRENT_SESSION)), PageRequest.of(0, 25), 1));

		Page<MeController.SessionDto> page = controller.sessions(0, 25);

		assertThat(page.getContent()).extracting(MeController.SessionDto::id)
				.containsExactly(CURRENT_SESSION);
		var pageable = forClass(Pageable.class);
		verify(me).sessions(eq(USER_ID), pageable.capture());
		assertThat(pageable.getValue().getPageNumber()).isZero();
