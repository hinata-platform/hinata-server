 ...
 * their own and are given up after the timeout. There is no queue: a lookup that hangs
 * on a silent name server keeps its thread until the operating system gives up, and a
 * queue behind a few of those would make every other request wait for them.
 */
public final class PublicDns implements Dns {
	 ...
	 *
	 * @throws SlowLookup           when the lookup takes longer than the timeout, or
	...
	/** The lookup took longer than the timeout, or could not start because too many are still running. */
