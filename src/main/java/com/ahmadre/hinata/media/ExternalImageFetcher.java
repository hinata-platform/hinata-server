...
/**
 * Fetches the organization logo from the external URL an administrator configured,
 * <b>server-side</b>, so that clients load it from our own origin through
 * {@code /api/v1/meta/logo}: Flutter web's CanvasKit taints on a cross-origin
 * {@code <img>} and silently drops it, and PDF exports and mails need the bytes here.
 *
 * <p>An administrator types the address, but the endpoint that serves the result is
 * public and the host behind the address can change its answers at any time, so the
 * fetch is locked down as if anybody had chosen it (OWASP A10):
 * <ul>
 ...
 *   <li>the response must be an image type a browser renders safely;</li>
...
public class ExternalImageFetcher implements DisposableBean {
	...
	public StorageService.StoredObject fetchLogo(String rawUrl) {
		...
		for (int hop = 0; hop <= MAX_REDIRECTS; hop++) {
			try (Response response = send(url, deadline)) {
				...
				if (!DISPLAY_TYPES.contains(contentType)) {
	...
	private Response send(HttpUrl url, long deadline) throws IOException {
		...
		return dns.within(deadline, call::execute);
	}
	...
	private static ApiException notAllowed(HttpUrl url) {
		log.warn("Blocked SSRF-prone logo address: {}", url.host());
